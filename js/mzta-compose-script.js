...
switch (message.command) {
  ...
  case 'showTemporaryMessage': {
    ...
    const variant = (message.variant ?? 'info').toString().toLowerCase();
    const duration = Number(message.duration) > 0 ? Number(message.duration) : 5000;

    const doc = window.document;

    if (!doc.getElementById('thunderai-temp-message-style')) {
      const style = doc.createElement('style');
      style.id = 'thunderai-temp-message-style';
      style.textContent = `
        #thunderai-temp-message-container {
          position: fixed;
          top: 16px;
          right: 16px;
          display: flex;
        ...
        .thunderai-temp-message.visible {
          opacity: 1;
          transform: translateY(0);
        }

        .thunderai-temp-message.thunderai-temp-info {
          background: rgba(27, 94, 169, 0.92);
        }

        .thunderai-temp-message.thunderai-temp-warning {
          background: rgba(180, 86, 24, 0.92);
        }

        .thunderai-temp-message.thunderai-temp-success {
          background: rgba(22, 125, 72, 0.92);
        }
    ...
    let container = doc.getElementById('thunderai-temp-message-container');
    if (!container) {
      container = doc.createElement('div');
      container.id = 'thunderai-temp-message-container';
      doc.body.appendChild(container);
    }

    const toast = doc.createElement('div');
    toast.className = `thunderai-temp-message thunderai-temp-${variant}`;
